...
I have been promising some people that this (my first) post will be out for quite some time now. It has finally arrived and the main reasons for slowness are:
...
While we only need the quarter-by-quarter scores of each game, only the `player_stats` data set has the required quarter by quarter score fields on it for some reason. Thus, we will start by creating a 'flattened' version of the players data set that provide game-level field.
...
player_stats_afltables_tb |> 
  group_by(
    ...
    Away.team, Home.score, Away.score, HQ1G, , HQ1G
  )
results_afltables_tb <- as_tibble(results_afltables)
```
