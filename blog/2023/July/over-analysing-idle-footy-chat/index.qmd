...
description: |
  This blog post discusses the types of statistical questions one often posits while watching the footy (or indeed any sport). I will use this as a medium through which I can introduce analysis of AFL data in R.
date: 2023-07-22
categories: 
  - Sport
  - AFL
...
[^footy]: As a Western Australian this is how I define it (along with the majority of Australia) but I am aware that this term is reserved for rugby league in New South Whales and Queensland
...
* Trawling through more obscure websites such as [AFL Tables](https://afltables.com/afl/afl_index.html){target="_blank"} to answer things liek "what is the most disposal Zac Dawson had in a game"[^zac-dawson]; or
...
However if you are a [gadget-type operator](https://www.youtube.com/clip/UgkxtEJxG9BEEvFMmGfpjSBWc4MtWPKwNTLW){target="_blank"}[^tipping-name] like myself, you will expand the number of footy stats questions you can answer immensely by accessing and manipulating the raw data yourself. There are of course a multitude tools and approaches to this but in this post, I will be using R ( my preferred programming language).
...
1. It adds something interesting to a game that otherwise lacks excitment

2. These games are often filled large statistical feats that might be close to records

If we also consider that it was two actuaries talking, it is safe to say that footy stats questions were flying left, right and center on that gloomy (but not because of the weather) Saturday night at the SCG. The questions we posed did not just vanish into the ether either, I (with the idea of writing this article in mind) decided to record the more interesting (and doable) question listed below:
...
- What is the record for the most 5+ goal kickers in a single game (i.e. the most bags)?
...
- Has anyone won a Norm smith in their first season in the AFL?
...
- What is the worst win-lost record to make finals?*

::: {.callout-note}

## Disclaimer on the asterisk

I have put an asterisk questions cannot easily be answered by the methods I discuss below and others are too lengthy for this blog post[^future-posts]. The remaining questions will be tackled using R below.

:::

[^future-posts]: These may get get their own blog post in the future.

If you aren't interested in the code but what really interests you is the answer, you can jump ahead to by [clicking here](#summary-of-answers).
...
We can obtain the data we need very simply using ts name-sake, the `fitzRoy` R package. It abstracts away all the web scraping and api calls for us into a very helpful family of `fetch_*` functions.
...
The so-called *pipe operator* (`%>%`) of the [`magrittr`](https://magrittr.tidyverse.org/){target="_blank"} has been a core staple of tidyverse since its inception but since the R core team the a so-called *native pipe* was added to `base` R (`|>`) (in version [4.1](https://www.jumpingrivers.com/blog/new-features-r410-pipe-anonymous-functions/){target="_blank"}[^function-shorthand]), this has lead to a split in adoption. There are some nuances in its usage[^future-pipe-post] but it overall behaves in a similar way to the `magrittr` pipe and has [less overhead (and is therefore faster)](https://stackoverflow.com/questions/67633022/what-are-the-differences-between-rs-new-native-pipe-and-the-magrittr-pipe){target="_blank"}. The native pipe was initially missing some of the key features of the magrittr pipe, new features[^pipe-features] have been added to it that (in my mind) mean that it might have even surpassed the magrittr pipe.
...
> Note that the common shortcut `Ctrl+Shift+M` can be changed from the `magrittr` pipe (`%>%`), which is still the default, to the native pipe (`|>`).

[^function-shorthand]: another cool thing introduced in this version of R was 

[^future-pipe-post]: I may even cover these in a future blog post

[^pipe-features]: 1. In R version 4.2, `_` was  added as a placeholder character
  2. In R version 4.3,
  extractions using the `$` symbol are now allowed



### Webscraping package

While the majority of our data will be sources using the `fitzRoy` package, a small amount of data (namely Norm Smith medalists) which is outside of the scope of `fitzRoy` will require us to perform some bespoke web scraping. This will be performed using the `rvest` package.
