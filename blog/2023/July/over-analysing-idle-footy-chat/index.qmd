...
So let's begin by loading the package and while we're at it, I will also load all the other packages I will be using.
...
Where possible we will use the AFL Tables data set as it has the full history of the AFL on it. We note however, that the 
...
While we only need the quarter-by-quarter scores of each game, only the `player_stats_afl_tables` (and not `results_afl_tables`) dataset has the required quarter by quarter score data. Thus, we will start by creating a 'flattened' version of `player_stats_afl_tables` that provide game-level field.

First, let's take a look at the dataset and determine which fields are player-level and which are match-level.

```{r}
str(player_stats_afltables)
```

Inspecting the fields, we can determine that the following fields are player-level:

```{r}
player_level_fields <- c("First.name", "Surname", "ID", "Jumper.No.", "Playing.for", "Kicks", "Marks", "Handballs", "Goals", 
  "Behinds", "Hit.Outs", "Tackles", "Rebounds", "Inside.50s", "Clearances", "Clangers", "Frees.For", 
  "Frees.Against", "Brownlow.Votes", "Contested.Possessions", "Uncontested.Possessions", "Contested.Marks", 
