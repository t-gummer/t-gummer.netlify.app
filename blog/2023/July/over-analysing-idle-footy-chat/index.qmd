...
* Used fryzigg instead of AFL Tables (actually more complete). However AFL tables did allow me to link games very easily (this is also possible with fryzigg but would require additional work)
...
x <- fitzRoy::fetch_ladder_afltables(season = 2023, round_number = 20)
x <- fitzRoy::fetch_player_stats_fryzigg(season = 2023, round_number = 20)

fitzRoy_sources <- as_tibble(fread("fitzRoy_sources.csv"))

fitzRoy_sources |> 
  reactable(columns = list( # add conditional formatting
    data_source = colDef(name = "Data Source"), # add URL
    collection_method = colDef(name = "Collection Method"),
    level_of_detail = colDef(name = "Level of Detail"), # add level_of_detail_note tip
    speed_of_fetch = colDef(name = "Speed of Fetch"), # add speed_of_fetch_note tip
    update_frequency = colDef(name = "Update Frequency"),
    use_case = colDef(name = "Use Case"),
    URL = colDef(show = FALSE),
    level_of_detail_note = colDef(show = FALSE),
    speed_of_fetch_note = colDef(show = FALSE)
    ))

# AFL Website is slow because the data is very fragmented resulting in many api calls

# turn this into a nicesly colour coded reactable with those coloured circles that are conditionally formatted
