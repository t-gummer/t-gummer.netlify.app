...
# Player Stats:
player_stats_tb |> 
  mutate(full_name = paste0(First.name, " ", Surname, " (", Playing.for,")")) |> 
  filter(game_id == infamous_game_id) |> 
  reactable(
    # formatting code omitted
  ) 
...
# Match Stats:
reactable(match_stats_flat_dt[game_id == infamous_game_id, ], )# formatting code omitted
# Player Stats:
reactable(player_stats_dt[, player := paste0(First.name, " ", Surname, " (", Playing.for,")")]
          [game_id == infamous_game_id, ])# formatting code omitted
```
...
```{r}
#| echo: false
# TODO: add format and freeze game URL (open in a new tab)
match_stats_flat_tb |> 
  filter(game_id == infamous_game_id) |> 
  reactable_themed(
    columns = list(
...
```{r}
# TODO: add format and freeze player URL (open in a new tab) and name and team
player_stats_tb |>
  filter(game_id == infamous_game_id) |> 
  reactable_themed(
    columns = list(
