...
description: |
  This is a response to a rather lengthy feedback document I have received.
date: 2023-08-13
categories: 
  - bad
image: bt.jfif
...
Thanks for your detailed feedback on my article. I appreciate the time and effort you put into it and I have addressed a number of your points.

The general theme (with some exceptions) below is that I typically have fixed up spelling, grammar and punctuation errors but have left what I would (subjectively) describe as "intentional design choices" as they are.
...
[Got rid of it](../../index.html){target="_blank"}.

> verbose? hardly

It's all relative isn't it? Business writing (which is what most of the audience are used to) is not like this.
...
Yeah I think its a bit pointless too, I was just playing around with all the options I could include. Removed.
...
Add the comments section to the table of contents. Have to use some sort of login as this is a static site (no backend sever/databases are possible)
...
Can't do much about this, I too would like my footnotes to show up because such words are indeed fun but alas.

> conversational tone taken throughout has divested your good sense on where to place commas, this will make up the bulk of adjustments. writing style leaves something to be desired but will endeavour to avoid correcting this, we know you have your ways. most of the comma-based corrections will place the comma inside the example text, while wording corrections will explain their placement outside the quotation to preserve easy <kbd>ctrl</kbd>-<kbd>f</kbd>-ing

* Love a conversational tone. It is what a lot of people have remarked that they liked about it.

* However the placement of commas is important (I do it by guessing mostly).

* I think the writing style doesn't leave "something to be desired", it is optimally desirable for me. But I would be open to specific examples where you think it could be improved, there is no need to refrain.
...
More viewers than this according to analytics, the YouTube clicks aren't the best measure because most people wouldn't bother to click on them.
...
I previously had it later on but it felt like it killed the flow of the article. Perhaps I can add it to the end somewhere but it might be a bit weird.
...
Added the comma (I must admit I have no idea when a comma should be added before a 'but'). However, I have not rewritten it.
...
Amended (punctuation is not my friend).
...
The verbose footnotes are a key feature of the style I am going for this article. Call me a maverick if you like but this is a convention I simply refuse to conform to!


```{=html}
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/ZvZ2xRxdOWI?start=66" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
```


> ""the most data rich sport on Earth"2," should be a hyperlink

I think I'll stick with the footnote but I will admit there is some inconsistency with how I approach referring to external sources. Going back to your previous note I would have though this would be a more appropriate use of a footnote than my other uses anyway.
...
Placed the "you might" ask outside of the quotes.
...
Isn't an emdash used as a substitute for brackets —like so—? I've chucked a colon in there.

Nice peep show reference.

```{=html}
<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/PiLVAz-Jczg" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
```
...
Fixed the capitalisation, I can't hyperlink this one unfortunately but have attached a screenshot you can hover over and see.
...
I like this, thanks (I worded it slightly differently but really like the "even so,")
...
This is a good point but at this point I think it probably wouldn't be worth rewriting. However, I will consider this as a general principal for future reference.
...
Incidentally where should however be used if not at the beginning of a sentence?
...
I put other inside the hyperlink to emphasise that I was linking to other quotes that were not that one.
...
Sure, I'll go with that. But I also thought you hated flavour!
...
Removed the quotes but not going to both moving it to the top. To be honest I kind of use quotes in some places purely because I like the look of them.
...
Are you a fan of the Oxford comma? I did find this sentence a bit difficult to write and rewrote it a few times and was never quite happy with it. Have tried to implement what you have said as best I can. Although I like (and kind of miss) "if we also consider".
...
I initially adopted this approach but as it requires a more detailed explanation involving bullet points (which I have discovered don't work well with footnotes), I went for a callout box instead.
...
Yeah I think I did slip into a bit of a change of tone when I stated talking about technical things but I don't really want to change it at this point.
...
Nice, did you comment about it like people commenting "first" on YouTube videos.
...
Yeah can't do much about the double click thing as it is just the library I was using. However it is logical to start it in alphabetical order so I have done so.
...
Yeah I agree, I thought about changing it when I saw it but just wanted to get the article out (I wrote it out of order and that was one of the last bits I did).

*Unique* is misspelt because I can't spell or type properly.

Also thanks for calling my table "otherwise lovely", I'm blushing.

> 
> row "Squiggle", column "Speed of Fetch" rollover pop-up text: "indended to be slow" - was this indendional?

Nope, thanks for the creative way of pointing out a typo.
...
Have re-written the sentence a bit, it didn't need brackets, it was just poorly worded (grammatically).

R users would know what it is and why they should care.

It is not orange because it is formatted as it is generated by a feature that automatically links functions (if formatted as code) to their documentation but in some cases I link them elsewhere if I am not happy with this, I might be able to change that behaviour but it would require fiddling with my custom CSS some more and for now I think it's fine.
...
Again R users would understand what I was talking about (it will make the code less efficient and more verbose and clunky).
...
Indeed I was kind of almost telling myself that I would write this or that later when I wrote those parts. I see the point but I'll keep it as it is because its more effort than its worth to change.
...
Don't be snarky, its very easy to glaze over things like that. I assure you, there were a lot of things I fixed before it reached your eyes!
...
I have mentioned this before, I don't mind this inconsistency, it makes sense to me.

But that is pretty cool, how did you learn Javascript? That isn't a very optimal way of doing it I don't think and it also would interact weirdly with dark mode, I think just using CSS would be cleaner. 

I kind of want to play around with the code though, although I often get frustrated whenever I have to add JS to something, I never managed to get it to run properly on the first or even the 10^th^ attempt half the time.
...
Important to distinguish this from the `fitzRoy` package as it is a different way of obtaining data.
...
Oh how nice, I get a bit of a breather.
...
Its just checking that the results are consistent (`TRUE` means they are). R users should understand this and I have commented my code appropriately (it is quite deliberate and not an error).
...
Not an error, as I stated above. Henceforth I will mark my response to these erroneously labelled error with "output verification non-error". 
...
Hmmm interesting idea, I think I will leave it as is. The point is to illustrate what data is available, not to present it nicely so its meant to have those kinds of titles. I think it sufficiently serves the purpose of showing you what is available.
...
You know I haven't used "irregardless" in like 5 years right?
...
Because I don't like the fact that wikipedia does that, I want to be able to go back to it at any point in the article.
...
Yeah I agree it isn't ideal, I have fixed the opposition one, but more finessing than this feels like its more than its worth for me (it is a rather manual process).. The alignment is perfectly logical, numerical data is right aligned and text data is left aligned (as they both should be respectively).

Overall I find the tables quite readable but maybe that's just me.

> "highest scoring quarter" section, first tabs 1TRUE

Output verification non-error.
...
Not sure where the comma ought to be so left that one be.
...
Fair dos, have change it to "accelerator" instead of "gas".
...
Yeah that one still gets a fair bit of air time on the television to this day, whenever they have one of those segments where they try to humiliate one another with moments from their playing days.
...
Ahhh I see you are a fan of the Oxford comma.

I'm keeping that paragraph.

> "...unbelievable goals like this." still got the edit of you kicking a similar goal down the park somewhere

Ahahaha my favourite, I thought that footage was lost. Can you please send it to me?
...
Got rid of the a little, just wen with "although sad".

Nah I like (over-)explanation, keeping it.
...
I actually got rid of this sentence, as it turns out they do not make the list of most multiple goal kickers after all (they're up there but not in the top 5).
...
Noticed the "its" while reading the quote you provided (seems like I am learning).

Fixed both "its" and "thought" (that was a tricky pickup, thanks!)

I'm leaving the word notched because I like it. Given the number of times I repeat the words "note" and "interestingly", I think it[']s the least of my concerns.
...
Fixed (paraphrased as to keep my academic integrity in tact).
...
The third quarter is often known as the "premiership quarter", some believe it is the quarter that is most important for a team's winning chances, perhaps because it comes after half time, where a coach has the opportunity to address his players for an extended time and instruct them on adapting to the opposition's tactics.

I have added a footnote with a hyperlink to prevent confusion.
...
Not sure what you are referring to when you say torturous here.

> make sure to ctrl-f aforementioned and remove some of the 7 hyperlinks

Nah leaving it, <kbd>CTRL</kbd>+<kbd>F</kbd>ing actually made me consider adding even more hyperlinks (be careful what you ask of me).
...
Its from google haha, just gonna leave it as is but the symmetry might be nice.
...
Yeah I know, it would seem the footnote messes witht he quote formatting, not much I can do.

Actually, I fixed it by no longer making it a footnote.
...
Because in this case the team I was targeting in this case was West Coast instead of Sydney because they were the ones who played poorly. I think this section showed that it takes two to tango and in fact a little extra help from the elements in most cases as well.
...
It is what it is, I don't read so I don't know any better.
...
That makes me want to keep it, I like the contrast and most people don't even know what it means. I think torrid sounded a bit like torrential to me so its true meaning is a surprising revelation to me.
...
Yeah okay don't really want to reword the entire thing but year it is a rather long sentence I will admit (probably could struggle to say it in one breath). Added the commas.
...
Oh no who's versus whose (I don't reckon I will ever learn this one, I suppose whose being of possession makes sense but whatever happened to the apostrophe of possession)

I'm keeping "however it was" because I see nothing wrong with it, albeit I do like the word "albeit"
...
I think in this case I split a sentence in two thinking it was too long but I know now to trust my instinct and to always write long sentences and seldom use full stops (thanks!).

Lol you and your commas, fine.
...
Lol okay sure thing.
 ...
 remember this sentence being a pain to write (describing things is hard).
...
I think I'm done with writing but will keep this in mind for the future.

> find and replace "interestingly" with synonyms, rewrite accordingly

I think I'll leave it and keep it in mind.
...
Annoyingly, they forgot the "ly" in the search results
...
I don't see a problem with it. What is your grip?
...
I'm trying to buck the trend of low attention spans I suppose, we'll see if it pays off.

And yes 66 minutes is a rather inaccurate estimate but I kind of like how high it is, feels like it reflects the amount of time I put into it (although one could also use a certain Mark Twain quote to rebuke me here)
