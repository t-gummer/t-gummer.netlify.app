...
The out-of-control scope has also impacted the length of this article. I commend anyone who actually reads the whole thing but hopefully its also possible to jump around and only read the parts that interest you in particular using the table of contents. I will endeavour to keep future blog posts a lot shorter going forward.
...
I recently had the misfortune ([as a West Coast fan](https://www.instagram.com/p/CiRdBD9A4lU/){target="_blank"}) of attending a game live between the Sydney Swans and the (not so) mighty West Coast Eagles with the following scoreline:
...
As the questions we are trying to answer
...
[^source-champion-data]: [Source (Champion Data)](https://www.championdata.com/glossary/afl){target="_blank"}
...
player_stats_tb |> 
  ...
  arrange(desc(clangers)) -> most_clangers_tb
      ...
      match_round = colDef(
        cell = function(cell_value, row_index) {
          most_clangers_tb |>
            slice(row_index) |>
            pull(season) -> season
          paste("match_round", cell_value, season)
          }
        ),
      rank = colDef(name = "Rank"),
...
Also notable is one of three games played in Shanghai as part of the AFL's attempt at entering into the Chinese market between 2017 and 2019. While there was some rain about, one might presume from this that the Suns and Port didn't put on a particularly impressive display of our game on that occasion, either that or they were playing the Richmond-inspired high-octane footy before a most other teams had started doing it.
...
* Seeing a game in 2018 is also a fun reminder of the last time the Eagles won a premiership which feels a long way off given the current predicament the club is in, in spite of their recent win against the also similarly languishing[^roos-optimism] Roos last Sunday.
...
* We can't entirely blame the players, given the conditions the conditions. Anyone who has kick a footy around in the wet will know how much heavier and slipperier than usual it can get (it is often described as being like a bar of soap). By looking through old highlights packages of the top several games in this metric, all of them appear to have impacted significantly by weather conditions.
...
### Background on the Norm Smith

* The Norm Smith Medal is awarded to the player who is adjusted best on ground in the AFL grand final. 
...
As previously mentioned, norm smith data is not available on FitzRoy, so we will have to scrape it with some of our own bespoke code[^illustrates-fitzroy-point]. The AFL website conveniently has a nice table, listing all the winners since the award began in 1979.


[^illustrates-fitzroy-point]: This illustrates the fact that sometimes you need to stray outside of `fitzRoy` but it give most of the footy data you could ever want.
...
norm_smith_tb |> 
  left_join(
    player_details_tb |> select(Player, Team, Games),
    by = c("Player" = "Player", "Club" = "Team")
    ) |> View()
